...
def _write_node_index(
    ...
    # Every rendered document and interior-child navigation row consumes one
    # pagination unit. The section index keeps mirrored entries assigned to the
    # correct sibling without relying on value equality.
    render_units: list[tuple[int, dict[str, Any] | None]] = []
    total_document_count = 0
    for section_index, (_, entries, is_child_interior, _) in enumerate(section_blocks):
        render_units.extend((section_index, entry) for entry in entries)
        total_document_count += len(entries)
        if is_child_interior:
            render_units.append((section_index, None))

    per_chunk = max(1, page_size)
    chunk_count = max(1, (len(render_units) + per_chunk - 1) // per_chunk)
    written: list[str] = []
    ...
    if node_dir.resolve() != target.resolve() and not node_dir.resolve().is_relative_to(target.resolve()):
        raise WikiWriteError("node_dir_escapes_target", f"node dir escapes target: {node_dir}")
    ...
    for chunk_index in range(1, chunk_count + 1):
        ...
        chunk_end = min(chunk_start + per_chunk, len(render_units))
        chunk_units = render_units[chunk_start:chunk_end]
        indexed_count = sum(entry is not None for _, entry in chunk_units)
        body = _node_index_body(
            source_name, source_rel, node_path,
            section_blocks, chunk_units, indexed_count,
        ...
        page = WikiPage(
            ...
            frontmatter=_frontmatter(
                ...
                total_group_count=total_document_count,
...
def _node_index_body(
    ...
    node_path: tuple[str, ...],
    section_blocks: list[tuple[str, list[dict[str, Any]], bool, str]],
    chunk_units: list[tuple[int, dict[str, Any] | None]],
    indexed_count: int,
    chunk_index: int,
    chunk_count: int,
) -> str:
    lines = [
        ...
        f"- Documents on this page: {indexed_count}",
        f"- Page chunk: {chunk_index}/{chunk_count}",
        "",
    ]
    units_by_section: dict[int, list[dict[str, Any] | None]] = {}
    for section_index, entry in chunk_units:
        units_by_section.setdefault(section_index, []).append(entry)

    for section_index, section_units in units_by_section.items():
        child, _, _, child_index_rel = section_blocks[section_index]
        child_lines: list[str] = [f"## {child}", ""]
        entry_index = 1
        for entry in section_units:
            if entry is None:
                child_lines.append(f"- → [[{child_index_rel}|{child}/_entries]]")
                child_lines.append("")
                continue
            child_lines.extend(_entry_lines(entry_index, entry))
            entry_index += 1
