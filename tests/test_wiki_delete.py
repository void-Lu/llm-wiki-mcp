...
def delete_root(tmp_path: Path) -> Path:
    root = tmp_path / "vault"
    # Raw source
    raw = root / "raw" / "sources" / "file" / "proj" / "my-source"
    raw.mkdir(parents=True)
    (raw / "snapshot.json").write_text("{}", encoding="utf-8")

    # Wiki pages derived from this source
    wiki = root / "wiki"
    project_pages = wiki / "projects" / "proj" / "architecture"
    project_pages.mkdir(parents=True)
    (project_pages / "derived-page.md").write_text(
        "---\ntype: architecture\ntitle: Derived Page\ngenerated: true\nproject: proj\nsources:\n- raw/sources/file/proj/my-source/snapshot.json\n---\n\n# Derived Page\n\nContent.\n",
        encoding="utf-8",
    )
    # Manual page (should not be deleted)
    source_pages = wiki / "sources" / "projects" / "proj" / "architecture"
    source_pages.mkdir(parents=True)
    (source_pages / "manual-page.md").write_text(
        "---\ntype: source_index\ntitle: Manual Page\nsources:\n- my-source/other.ts\n---\n\n# Manual Page\n\nManual.\n",
        encoding="utf-8",
    )
    # Source summary
    (source_pages / "my-source.md").write_text(
    ...
    # Ingest cache
    cache = root / ".llm-wiki" / "ingest-cache" / "file" / "proj"
...
def test_dry_run(delete_root: Path):
    ...
    assert (delete_root / "raw" / "sources" / "file" / "proj" / "my-source").exists()
...
def test_delete_cascades(delete_root: Path):
    ...
    # Raw dir gone
    assert not (delete_root / "raw" / "sources" / "file" / "proj" / "my-source").exists()
    # Derived page gone
    assert not (delete_root / "wiki" / "projects" / "proj" / "architecture" / "derived-page.md").exists()
    # Manual page preserved
    assert (delete_root / "wiki" / "sources" / "projects" / "proj" / "architecture" / "manual-page.md").exists()
    # Source summary gone
    assert not (delete_root / "wiki" / "sources" / "projects" / "proj" / "architecture" / "my-source.md").exists()
    # Cache cleaned
    assert not (delete_root / ".llm-wiki" / "ingest-cache" / "file" / "proj" / "my-source.json").exists()
...
def test_delete_source_preserves_generated_pages_with_other_sources(delete_root: Path):
    shared_page = delete_root / "wiki" / "projects" / "proj" / "architecture" / "shared-page.md"
    shared_page.write_text(
        ...
        "project: proj\n"
        "sources:\n"
        "- raw/sources/file/proj/my-source/context.json\n"
        "- raw/sources/file/proj/other-source/context.json\n"
        "---\n\n"
