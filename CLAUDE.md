...
- 启动 MCP server：`netsuite-llm-wiki-mcp-server` 或 `python -m netsuite_llm_wiki_mcp.server`
...
- 不要在代码、测试或文档中硬编码个人 Vault 路径、API key、token、邮箱、手机号等敏感信息；脱敏逻辑在 [src/netsuite_llm_wiki_mcp/redaction.py](src/netsuite_llm_wiki_mcp/redaction.py)。
