...
- [wiki_io.py](src/wiki/wiki_io.py) 的 `refresh_page_retrieval` 与 chat projection 只调用 RetrievalIndexStore 的单页 `update_page`；删除/改名使用对应增量操作。`refresh_navigation`/`refresh_overview` 在 page mutation 传入 `changed_path` 时只投影受影响的 project/domain/entity 导航范围和 overview 聚合，缺目录或结构异常稳定失败进入 `repair_pending`；repair page-operation 重放时对这类导航/概览结构缺口显式升级为无 hint 的全量重建，并将升级原因写入 operation journal；不传 hint 则保留全量重建行为。[wiki_index.py](src/wiki/wiki_index.py) 的 `rebuild_retrieval_index`/`refresh_indexes` 只用于显式初始化、CLI/admin 或兼容测试，普通 MCP 写入不得隐式全量建库。检索索引缺失或不兼容时返回 `rebuild_required` 和 `rebuild_retrieval_index` repair action。
